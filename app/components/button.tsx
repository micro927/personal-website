...
function Button(props: ButtonProps) {
  ...
  const buttonVariantMapping = {
    [VariantEnum.PRIMARY]: 'bg-primary-500 hover:bg-primary-600',
  ...
  return (
    <button
      {...props}
      type="button"
      className={`rounded-xl px-4 py-3 font-medium transition duration-200 ${buttonVariantMapping[variant]} ${props.className}`}
    />
  );
}
